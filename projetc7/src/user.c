...
////////////////////////////
int chercherr211(char util[],char pass[])
{
FILE *f;
user u;
int x=0;
 ...
 if(f!=NULL)
    {
	while(fscanf(f,"%s %s %s\n",u.userr,u.pass,u.rol)!=EOF)
	{ 
 	 if  ( (strcmp(util,u.userr)==0) && (strcmp(pass,u.pass)==0)  )
			{if (strcmp(u.rol,"admin")==0)
				x=1;
			if (strcmp(u.rol,"technicien")==0)
				x=2;
			if (strcmp(u.rol,"agentf")==0)
				x=3;
			if (strcmp(u.rol,"agentr")==0)
				x=4;
			if (strcmp(u.rol,"nutritioniste")==0)
				x=5;
			if (strcmp(u.rol,"etudiant")==0)
				x=6;
